...
//    waitForOperationStatus( bucket.couchbaseCluster.replace(key, exp, value, persistTo, replicateTo), bucket, ec ).map(OpResult(_, 1, Some(Json.parse(value))))(ec)
//  }
//
//  /**
//   *
...
//    waitForOperationStatus( bucket.couchbaseCluster.add(key, exp, value.getBytes, persistTo, replicateTo), bucket, ec ).map(OpResult(_, 1, Some(Json.parse(value))))(ec)
//  }
//
//  /**
//   *
...
//   * @return the operation status
//   */
//  def javaAddBlob[T](key: String, exp: Int, value: Array[Byte], persistTo: PersistTo, replicateTo: ReplicateTo, bucket: CouchbaseBucket, ec: ExecutionContext):  Future[OpResult] = {
//    waitForOperationStatus( bucket.couchbaseCluster.add(key, exp, value, persistTo, replicateTo), bucket, ec ).map(OpResult(_, 1, Some(Json.parse(value))))(ec)
//  }
//
//  /**
//   *
...
//    waitForOperationStatus( bucket.couchbaseCluster.set(key, exp, value, tc), bucket, ec ).map(OpResult(_, 1))(ec)
//  }
//
//  /**
//   *
...
//    waitForOperationStatus( bucket.couchbaseCluster.set(key, exp, value, persistTo, replicateTo), bucket, ec ).map(OpResult(_, 1, Some(Json.parse(value))))(ec)
//  }
//
//  /**
//   *
...
//    waitForGet( bucket.couchbaseCluster.asyncGet(key), bucket, ec ).flatMap {
...
//    waitForGet( bucket.couchbaseCluster.asyncGet(key, tc), bucket, ec ).flatMap {
//      case value: T@unchecked => Future.successful(value)
...
//    waitForGet( bucket.couchbaseCluster.asyncGet(key), bucket, ec ).map { f =>
...
//    waitForHttp( bucket.couchbaseCluster.asyncQuery(view, query), bucket, ec ).map { results =>
...
//    waitForHttp[View]( bucket.couchbaseCluster.asyncGetView(docName, viewName), bucket, ec )
//  }
//
//  /**
//   *
